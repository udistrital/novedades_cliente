...
angular
    ...
    .controller(
        ...
        function (
            ...
            self.cargarCdprp = function () {
              self.cdp_numero = 0;
              self.rp_numero = 0;
            
              if (!self.unidadEjecutora) {
                console.warn("[ACTA SUSP] unidadEjecutora aún no existe, NO consulto CDP/RP");
                return;
              }
        
              var unidadParam = String(self.unidadEjecutora).padStart(4, "0");
              var path =
                "cdprptercerocontrato/" +
                self.contrato_vigencia + "/" +
                self.contrato_id + "/" +
                unidadParam + "/12";          
              financieraJbpmRequest.get(path).then(function (res) {            
                var arr = res && res.data && res.data.cdp_rp_tercero && res.data.cdp_rp_tercero.cdp_rp;
                if (arr && arr.length) {
                  var last = arr[arr.length - 1];
                  self.cdp_numero = last.cdp || 0;
                  self.rp_numero = last.rp || 0;
                } else {
                }
              }).catch(function (err) {
              });
            ...
            agoraRequest
                ...
                .then(function (agora_response) {
                    if (agora_response.data.length > 0) {
                        ...
                        self.contrato_obj.valor = String(
                            agora_response.data[0].ValorContrato
                        );
                        ...
                        amazonAdministrativaRequest
                            ...
                            .then(function (scd_response) {
                                ...
                                swal({
                                    title: $translate.instant('TITULO_ERROR_LEGAL'),
                                    type: 'error',
                                    html: "Error al consultar datos de supervisor del contrato " + self.contrato_obj.numero_contrato +
                        ...
                        agoraRequest
                            ...
                            .then(function (jj_response) {
                                        ...
                                        coreAmazonRequest
                                            ...
                                            .then(function (scj_response) {
                                                ...
                                                self.contrato_obj.jefe_juridica_tipo_documento =
                                                    ijpn_response.data[0].TipoDocumento.ValorParametro;
                                                self.contrato_obj.jefe_juridica_nombre_completo =
                                                    ijpn_response.data[0].PrimerNombre +
                                                    " " +
                                                    ijpn_response.data[0].SegundoNombre +
                                                    " " +
                                                    ijpn_response.data[0].PrimerApellido +
                                                    " " +
                                ...
                                swal({
                                    title: $translate.instant('TITULO_ERROR_LEGAL'),
                                    type: 'error',
                                    html: "Error al consultar datos de jefe de contratación del contrato " + self.contrato_obj.numero_contrato +
                                        $translate.instant('ANIO') + self.contrato_obj.vigencia + '.' + error,
                                    showCloseButton: true,
                                    showCancelButton: false,
                                    confirmButtonText: '<i class="fa fa-thumbs-up"></i> Aceptar',
            ...
            self.postNovedad = function (nuevoEstado, docDefinition, dateTime, enlaceDoc) {
                ...
                novedadesMidRequest
                    ...
                    .then(function (vc_response) {
                        ...
                        //Servidor no disponible
                        var alert = "Error al validar el cambio de estado del contrato # ";
                        swal({
                            title: $translate.instant("TITULO_ERROR_ACTA"),
                            type: "error",
                            html: alert +
          ...
          self.ejecutarGeneracionActa = function () {
                                ...
                                .then(function (ce_response) {
                                    if (
                                        ...
                                        swal(
                                            ...
                                            $translate.instant(
                                                "El contrato no se encuentra en ejecución debido a esto no se puede suspender."
                                            ),
                                            "info"
                                        ).then(function () {
            ...
            function NumeroASoloLetras(num) {
              num = parseInt(num, 10) || 0;
              if (num === 0) return "Cero";
              return String(Millones(num)).trim();
            }
           ...
           self.calculoPlazoLetras = function (plazo) {
            plazo = parseInt(plazo, 10) || 0;
            
            if (self.plazoDias) {
              var plazo_meses = plazo / 30;
              var res = String(plazo_meses).split(".");
              var cantidad_meses = parseInt(res[0], 10) || 0;
            
              var decimalStr = res[1] ? ("0." + res[1]) : "0";
              var cantidad_dias = Math.ceil(parseFloat(decimalStr) / 0.03333333333336);
            
              var meses_letras = NumeroASoloLetras(cantidad_meses);
              var dias_letras  = NumeroASoloLetras(cantidad_dias);
            
              if (cantidad_dias === 0) {
                return "(" + meses_letras + ") " + cantidad_meses + " " + $translate.instant("MENSAJE_MESES");
              }
            
              return "(" + meses_letras + ") " + cantidad_meses + " " + $translate.instant("MENSAJE_MESES") + " Y (" + dias_letras + ") " + cantidad_dias + " " + $translate.instant("DIAS");

            }
            
            var meses_letras = NumeroASoloLetras(plazo);
            return "(" + meses_letras + ") " + plazo + " ) " + $translate.instant("MENSAJE_MESES");
            };
                        ...
                        columns: [{
                            table: {
                                ...
                                body: [
                                    ...
                                    {
                                        text: "ACTA DE SUSPENSIÓN",
                                        bold: true,
                                        alignment: "center",
                                        fontSize: 9,
                                    },
                                    { text: "Código: GC-PR-014-FR-054", fontSize: 9 },
                                    {
                                        image: "logo_sigud",
                                        fit: [65, 120],
                                        margin: [3, 0],
                    ...
                    content: [{
                        ...
                        table: {
                            ...
                            body: [
                                ...
                                [
                                    { text: "CDP", bold: true, style: "topHeader" },
                                    {
                                        text: [
                                            { text: self.cdp_numero, bold: true },
                                        ],
                                        style: "topHeader",
                                    },
                                ],
                                [
                                    { text: "CRP", bold: true, style: "topHeader" },
                                    {
                                        text: [
                                            { text: self.rp_numero, bold: true },
                                        ],
                                        style: "topHeader",
                                    },
                                ],
                    ...
                    {   
                          ...
                          text: [
                            ...
                            "Entre los suscritos, de una parte, " +
                              (self.contrato_obj.ordenador_gasto_nombre_completo || self.contrato_obj.ordenadorGasto_nombre || "") +
                              ", identificado(a) con " +
                              (self.contrato_obj.ordenador_gasto_tipo_documento || "") +
                              " No. " +
                              ...
                              (self.contrato_obj.ordenador_gasto_rol || self.contrato_obj.ordenadorGasto_rol || "Ordenador del Gasto") +
                              " y ordenador del gasto, de otra, " +
                              (self.contrato_obj.contratista_nombre || "") +
                              " mayor de edad, identificado(a) con " +
                              (self.contrato_obj.contratista_tipo_documento || "") +
                              " No. " +
                              ...
                              " de " +
                              (self.contrato_obj.contratista_ciudad_documento || "") +
                              ", en su calidad de contratista, hemos acordado SUSPENDER el Contrato " +
                              (self.contrato_obj.tipo_contrato || "") +
                              " No. " + self.contrato_id +
                              " del año " + self.contrato_vigencia +
                              " durante el período comprendido entre el día " +
                              self.format_date_letter_mongo(self.f_inicio) +
                              " y el día " +
                              self.format_date_letter_mongo(self.f_fin) +
...

                        {
                          ...
                          ol: [
                        ...
                        
                            {
                              style: ["general_font"],
                              text: [
                                { text: "Que la cláusula 19- Suspensión del Contrato " + (self.contrato_obj.tipo_contrato || "") + " No. " + self.contrato_id + " del año " + self.contrato_vigencia + ", establece que ", bold: false },
                                { text: "“Las partes contratantes podrán suspender la ejecución del contrato, mediante suscripción de un acta en donde conste tal evento, cuando medie alguna de las siguientes causales. 1) por circunstancias de fuerza mayor o caso fortuito, debidamente comprobadas, que imposibiliten su ejecución. 2) por solicitud debidamente sustentada, elevada por una de las partes. El término de la suspensión no será computable para efecto del plazo de ejecución del contrato, ni dará derecho a exigir indemnización, sobre costos o reajustes, ni a reclamar gastos diferentes a los pactados en el contrato.”.\n\n",italics: true}
                              ...
                              stack: [
                                {
                                  text:
                                    ...
                                    ", solicita a " +
                                    (self.contrato_obj.supervisor_nombre_completo || "") +
                                    ", quien cumple la función de supervisor, la autorización para realizar la Suspensión del Contrato " +
                                    (self.contrato_obj.tipo_contrato || "") +
                                    " No. " + self.contrato_id +
                                ...
                                { text: (self.motivo || "_____________________"), },
                                { text: "\n" }
                              ]
                            },
                            ...
                            "Que mediante oficio No. " +
                              (self.numero_oficio_supervisor || "__________") +
                              " de fecha " +
                              ...
                              ", le comunicó a " +
                              (self.contrato_obj.ordenadorGasto_nombre || "") +
                              " en calidad de Ordenador del Gasto del citado contrato, la autorización para suspender el mismo, durante el período comprendido entre el día " +
                              self.format_date_letter_mongo(self.f_inicio) +
                              " y el día " +
                              self.format_date_letter_mongo(self.f_fin) +
                              ".\n\n",
                            ...
                            "Que por medio del oficio No. " +
                              (self.numero_oficio_ordenador || "__________") +
                              " de fecha " +
                              self.format_date_letter_mongo(self.fecha_oficioO) +
                              ", recibido por la Oficina de Contratación, " +
                              (self.contrato_obj.ordenadorGasto_nombre || "") +
                              ", como Ordenador del Gasto, solicitó la elaboración del acta de suspensión del contrato " +
                              (self.contrato_obj.tipo_contrato || "") +
                              " No. " + self.contrato_id +
                              " del año " + self.contrato_vigencia +
                              ", durante el período comprendido entre el día " +
...

                        {
                          ...
                          text: [
                            ...
                            { text: "el Contrato " + (self.contrato_obj.tipo_contrato || "") + " No. " + self.contrato_id + " del año " + self.contrato_vigencia + ", durante el periodo comprendido entre el día " + self.format_date_letter_mongo(self.f_inicio) + " y el día " + self.format_date_letter_mongo(self.f_fin) + ".\n\n" }
                          ]
                        },

                        {
                          style: ["general_font"],
                          text: [
                            { text: "CLAUSULA SEGUNDA: GARANTÍAS Y MECANISMO DE COBERTURA DE RIESGO - ", bold: true },
                            { text: "EL CONTRATISTA una vez se reinicie el contrato, deberá aportar una póliza de garantías donde se amplía el plazo de ejecución.\n\n" }
                          ]
                        },

                        {
                          ...
                          text: [
                            ...
                            { text: "En virtud de lo dispuesto en el Estatuto de Contratación – Acuerdo 003 de 2015 y las normas vigentes, se procederá a la publicación del presente documento en el SECOP II que administra la Agencia Nacional de Contratación Pública – Colombia Compra Eficiente.\n\n" }
                          ]
                        },
...

                        {
                          style: ["general_font"],
                          text: [
                            "En constancia de lo consignado en el presente documento, se firma en la ciudad de " +
                              (self.contrato_obj.ciudad_firma || "Bogotá D.C.") +
                              ", el día " +
                              self.format_date_letter_mongo(self.f_expedicion_acta) +
                              ".\n\n"
                          ]
                        },
