    ...
    function (
          ...
          if (agora_response.data.length > 0) {
            ...
            agoraRequest
              ...
              .then(function (acta_response) {
                self.contrato_obj.NumeroContrato =
                  acta_response.data[
                    acta_response.data.length - 1
                  ].NumeroContrato.Id;
                agoraRequest
                  .get(
                    "acta_inicio?query=NumeroContrato:" +
                    self.contrato_obj.NumeroContrato
                  )
                  .then(function (acta_response) {
            ...
            agoraRequest
              ...
              .then(function (acta_response) {
                ...
                self.contrato_obj.NumeroContrato =
                  acta_response.data[acta_response.data.length - 1].NumeroContrato.Id;

                const urlActa =
                  "acta_inicio/?query=NumeroContrato:" + self.contrato_obj.NumeroContrato;
                ...
                agoraRequest
                  ...
                  .then(function (acta_response) {
                    if (
                      !acta_response.data ||
                      !Array.isArray(acta_response.data) ||
                      acta_response.data.length === 0
                    ) {
                      return;
                    }

                    const fechaInicio = acta_response.data[0].FechaInicio;

                    const urlOrdenadorVigente =
                    ...
                    agoraRequest
                      .get(urlOrdenadorVigente)
                      .then(function (og_response) {
                        if (!og_response.data || og_response.data.length === 0) {
                          return;
                        }

                        const rolId = og_response.data[0].RolId;

                        const urlOrdenadorActual =
                          "ordenadores?query=RolId:" +
                          rolId +
                          "&sortby=FechaInicio&order=desc&limit=1";

                        agoraRequest
                          .get(urlOrdenadorActual)
                          .then(function (og_actual) {
                            if (!og_actual.data || og_actual.data.length === 0) {
                              return;
                            }

                            const ord = og_actual.data[0];
                            ...
                            const urlPersona =
                              "informacion_persona_natural?query=Id:" + ord.Documento;
                            ...
                            agoraRequest
                              ...
                              .then(function (iopn_response) {
                                ...
                                const persona = iopn_response.data[0];
                  ...
                  .catch(function (error) {
                    swal({
                      ...
                      type: "error",
                      html:
                        "Error al consultar ordenador del gasto del contrato " +
                        self.contrato_obj.numero_contrato +
                        " vigencia " +
                        self.contrato_obj.vigencia +
                        ".<br>" +
              ...
              .catch(function (error) {
                swal({
                  ...
                  type: "error",
                  html:
                    "Error al consultar contrato suscrito del contrato " +
                    self.contrato_obj.numero_contrato +
                    " vigencia " +
                    self.contrato_obj.vigencia +
                    ".<br>" +
      ...
      $scope.total_plazo_contrato = function (evento) {
        ...
        if (valor_prorroga <= valor_valido_prorroga) {
          ...
          var cantidad_dias = Math.ceil(
            parseFloat(decimal_cantidad_dias) / 0.03333333333336
          );
      ...
      self.calcularFechaFin = function (diasNovedad, setDate) {
        ...
        if (setDate) {
          ...
          var fecha = new Date(nuevaFechaFin);
          fecha.setDate(nuevaFechaFin.getDate() + 1);
        ...
        if (self.novedadOtrosi == false) {
          generateTipoNovedad(function (tiponovedad) {
            ...
            self.contrato_obj_replica = {
              ...
              Contratista: parseInt(self.contrato_obj.contratista),
              ...
              ValorNovedad: parseFloat($scope.valor_adicion.replace(/\,/g, "")),
              UnidadEjecucion: 205,
              TipoNovedad: parseFloat(220),
              esFechaActual: false,
            };
            if ($scope.adicion == true && $scope.prorroga == true) {
              self.contrato_obj_replica.NumeroCdp = parseInt(self.cdp_numero);
              self.contrato_obj_replica.VigenciaCdp = parseInt(self.cdp_vigencia);
          ...
          header: {
            ...
            table: {
              ...
              body: [
                ...
                [{
                  text: "MODIFICACIÓN: PRÓRROGA Y ADICIÓN No. 1 AL " +
                    self.contrato_obj.tipo_contrato.toUpperCase() +
                    " No. " +
                    self.contrato_id +
                    " DE " +
                    self.fecha_reg_ano +
